...
工具结果把 `upgrade_item_id` 放在每个升级项中，把 `upgrade_selection_id` 放在对应
候选中。当前媒体只返回分辨率、编码、HDR、大小、音轨摘要和字幕统计，不返回 Jellyfin
完整轨道对象。升级候选通过紧凑结果、稳定选择 ID 和分页限制规模；模型视图统一使用
单工具 Token 预算，但完整原始结果仍保存在 SQLite。
