...
1. 只有真实 115 文件操作返回 HTTP 405 时，OpenList 才记录风控标记；驱动缺少凭据时
   也会在本地状态中明确要求重新鉴权。
2. Core 每分钟读取 OpenList 本地状态；该读取不会请求 115。
3. 新的重新鉴权状态只创建一个扫码会话，同一张二维码通过所有已启用的管理员渠道发送一次。
4. Core 在二维码有效期内读取会话状态；管理员可直接扫描聊天图片，也可在 OpenList
   Storage 页面重新生成二维码。
5. 扫码确认后，OpenList 使用 Storage 原有客户端类型更新 Cookie。
